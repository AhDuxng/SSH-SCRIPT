...
Variability under impairment grows disproportionately for TCP-based protocols. In the High scenario, SSHv2's standard deviation reaches 706\,ms versus 202\,ms for SSH3, reflecting TCP's sensitivity to packet loss during multi-RTT handshakes. SSH3's QUIC handshake is less affected because a loss on the single round trip does not stall an unrelated cryptographic exchange in a separate round trip.
...
For W1, SSH3 achieves the lowest mean latency across all four scenarios (Table~\ref{tab:workload-latency}, W1 rows). The advantage is modest under clean conditions (81.6\,ms vs.\ 93.1\,ms for SSHv2 in VPN) but widens under impairment. In the High scenario, SSH3 averages 292.9\,ms, compared to 384.4\,ms for SSHv2 (1.31$\times$ faster) and 324.4\,ms for Mosh (1.11$\times$ faster). This reflects QUIC's lower per-request overhead: small payloads complete within a single congestion window, and 0-RTT request shipping eliminates an additional round trip that TCP-based SSH must pay for each command. Mosh's SSP frame delivery offers no benefit here because the output is too small to trigger frame coalescing.
