...
# CSV containing the VPN W4 recv_pct measurements.
# Override when running locally, for example:
#   VPN_W4_RECV_CSV=/path/to/w4_line_log_patched.csv python3 plot_paper_figures_recv_vpn.py
VPN_W4_RECV_CSV = os.environ.get(
    "VPN_W4_RECV_CSV",
    "/mnt/data/w4_line_log_patched(1).csv",
)
...
def collect_vpn_w4_recv_pct(csv_path=VPN_W4_RECV_CSV):
    """Mean recv_pct by protocol from the VPN W4 line log.

    Only rows with status=ok and numeric received_pct are used. This matches the
    summary logic used by the W4 benchmark, where failed rows have no valid
    received_pct.
    """
    ...
    if not csv_path or not os.path.exists(csv_path):
        print(f"[warn] VPN W4 recv_pct CSV not found: {csv_path}")
        return None
    ...
    if any(counts[p] == 0 for p in PROTOS):
        print(f"[warn] Incomplete VPN recv_pct data in {csv_path}: {counts}")

    return {p: (sums[p] / counts[p] if counts[p] else 0.0) for p in PROTOS}


def recv_pct_data():
    """Table V data plus VPN W4 recv_pct aggregated from w4_line_log_patched.

    The VPN row is added to Heavy-output because the uploaded CSV is a W4
    large-output benchmark log.
    """
    data = {
        "Lightweight": {
    ...
    vpn = collect_vpn_w4_recv_pct()
    if vpn is not None:
        data["Heavy-output"] = {"default": vpn, **data["Heavy-output"]}
    return data
...
def fig_recv_pct(rp):
    # Lightweight data still covers only controlled emulation scenarios.
    light_scen = ["low", "medium", "high"]
    light_labels = ["Low", "Medium", "High"]

    # Heavy-output includes the new VPN group when collect_vpn_w4_recv_pct()
    # successfully reads VPN_W4_RECV_CSV.
    heavy_scen = ["default", "low", "medium", "high"] if "default" in rp["Heavy-output"] else ["low", "medium", "high"]
    heavy_labels = ["VPN", "Low", "Medium", "High"] if "default" in rp["Heavy-output"] else ["Low", "Medium", "High"]
    ...
    # Heavy-output subplot — now includes VPN as the first group
