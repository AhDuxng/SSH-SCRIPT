...
COMMANDS = ["ls", "df -h", "grep -n root /etc/passwd", "git status"]
COMMAND_LABELS = {
    "ls": "ls",
    "df -h": "df -h",
    "grep -n root /etc/passwd": "grep passwd",
...
def load_scenario(scenario):
    path = DATA_ROOT / scenario / "w1_line_log.csv"
    values = defaultdict(list)
    with path.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            if row.get("status") != "ok":
                continue
            if (row.get("warmup") or "").strip() == "1":
                continue
...
def draw_scenario_chart(scenario, values):
    ...
    ax.set_title(f"W1 Command Latency - {SCENARIO_LABELS[scenario]}", fontsize=12, fontweight="bold", pad=12)
...
def main():
    summary_path = ROOT / "w1_updated_latency_summary.csv"
    with summary_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["scenario", "command", "protocol", "n", "mean", "std", "ci95_half_width", "source_file"])
        for scenario in SCENARIOS:
            source, values = load_scenario(scenario)
            fig = draw_scenario_chart(scenario, values)
            png = ROOT / f"w1_{scenario}_protocol_latency.png"
            pdf = ROOT / f"w1_{scenario}_protocol_latency.pdf"
