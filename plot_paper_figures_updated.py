...
def recv_pct_data():
    """Hard-coded from Table V (Lightweight, Heavy-output)."""
    return {
        "Lightweight": {
...
def add_network_group_brackets(ax):
    """Draw two x-axis group brackets: VPN vs controlled emulation.

    This is used for the session-setup figure, where the first scenario is the
    real VPN path and the remaining three scenarios are controlled impairment
    levels (Low/Medium/High). Coordinates use data units on x and axes-fraction
    units on y, so the annotation remains stable when the y-axis range changes.
    """
    trans = ax.get_xaxis_transform()

    def bracket(x0, x1, y, text, color):
        tick = 0.075
        ax.plot([x0, x1], [y, y], transform=trans, color=color,
                lw=2.2, clip_on=False)
        ax.plot([x0, x0], [y, y + tick], transform=trans, color=color,
                lw=2.2, clip_on=False)
        ax.plot([x1, x1], [y, y + tick], transform=trans, color=color,
                lw=2.2, clip_on=False)
        ax.text((x0 + x1) / 2, y - 0.085, text, transform=trans,
                ha="center", va="top", fontsize=FONT["tick"], color=color,
                clip_on=False)

    # Vertical separator between VPN and the controlled network scenarios.
    ax.axvline(0.5, color="0.65", linestyle="--", linewidth=1.0, alpha=0.8)

    # Scenario centers are 0, 1, 2, 3. The bounds below cover each group.
    bracket(-0.42, 0.42, -0.17, "VPN", COLORS["ssh"])
    bracket(0.58, 3.42, -0.17, "Controlled emulation", COLORS["ssh"])


# --- Figure builders ---

def fig_session_setup(means, stds):
    # Taller canvas and larger bottom margin are needed for the network grouping
    # brackets under the x-axis.
    fig, ax = plt.subplots(figsize=(6.8, 4.4))
    grouped_bar(ax, means, SCENARIOS, SCEN_LABELS,
                "Time (ms)",
                value_fmt="{:.0f}")
    ax.set_xlabel("")
    add_network_group_brackets(ax)
