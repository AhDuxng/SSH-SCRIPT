...
ssh "${SSH_ARGS[@]}" "${SERVER_USER}@${SERVER_HOST}" \
  "mkdir -p $REMOTE_QUOTED && command -v ${VIM_BIN:-vim} >/dev/null && command -v ${NANO_BIN:-nano} >/dev/null && command -v od >/dev/null && command -v fold >/dev/null && command -v sha256sum >/dev/null"
